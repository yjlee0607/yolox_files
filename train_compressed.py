...
def get_dataset(dir_path, name, image_set, transform):
    ...
    # ds = ds_fn(p, image_set=image_set,download=True, transforms=transform) # only for voc dataset
...
def evaluate(model, data_loader, device, num_classes):
    ...
    with torch.inference_mode():
        for image, target in metric_logger.log_every(data_loader, 100, header):
            ...
            # print(target.shape) # [1 520, 781]
            output = model(image)
            # print(output[0].shape, output[1].shape) # [1,21,480, 480], [1,21,480, 480]
